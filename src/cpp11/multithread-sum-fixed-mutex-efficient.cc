...
void partial_occupancy(std::vector<float> &det, size_t begin, size_t end) {
  // Here we scan our range and sum only to a local variable
  // first. We lock the global occupancy for as short a period as possible.
  size_t my_occupancy = 0;
  ...
  {
    // These extra braces are not strictly necessary, but they help make
    // it very clear what the scope of the lock_guard is
    std::lock_guard<std::mutex> lck(mtx);
    occupancy += my_occupancy;
  }
}
