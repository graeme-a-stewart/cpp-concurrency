...
int main(int argc, char *argv[]) {
  ...
  std::cout << "Serial loop took " << serial_tick_interval.seconds() << "s" << std::endl;

  // Do the parallel loop
  set_x(x, my_size);
  t0 = tbb::tick_count::now();
  // Here we just pass a lambda function that captures local variables by value: [=]
  tbb::parallel_for(tbb::blocked_range<size_t>(0, my_size), [=](tbb::blocked_range<size_t>& r) {
	for(size_t i=r.begin(); i!=r.end(); ++i)
	  x[i] = log(x[i]);
    });
  t1 = tbb::tick_count::now();
  auto parallel_tick_interval = t1-t0;
  std::cout << "Parallel loop took " << parallel_tick_interval.seconds() << "s" << std::endl;
  
  std::cout << "Parallel speedup: " << serial_tick_interval.seconds()/parallel_tick_interval.seconds() << "x" << std::endl;
